...
## conc 350.5 - 500.4  AQI 401 - 500
bplo_old <- c( 0.0, 12.1, 35.5,  55.5, 150.5, 250.5, 350.5, 500.5)
bphi_old <- c(12.0, 35.4, 55.4, 150.4, 250.4, 350.4, 500.4, 99999.9)
aqi_pm25_old <- data.frame(bplo_old, bphi_old, ilo, ihi, hcat)


calc_aqi_old <- function(df, pollutant_column = "pm25", pollutant = "pm25", round = "r") {
  
  if (pollutant == "pm25_est" | pollutant == "filt" | pollutant == "pm25") { 
    aqi_table <- aqi_pm25_old
  ...
  for ( i in 1:len) {
    if (!(is.na(aqi_table$bplo[i]))) {
      bpl <- aqi_table$bplo_old[i]
      bph <- aqi_table$bphi_old[i]
      il <- aqi_table$ilo[i]
      ih <- aqi_table$ihi[i]
      hc <- as.character(aqi_table$hcat[i])
      
      
      df$aqi_old[!is.na(df$poll) & df$poll >= bpl & df$poll <= bph] <- 
        true_round((((df$poll[!is.na(df$poll) & df$poll >= bpl & df$poll <= bph] - bpl) *((ih - il)/(bph - bpl))) + il),0)
      df$hcat_old[!is.na(df$poll) & df$poll >= bpl & df$poll <= bph] <- hc
      
    }
    
  }
  df$aqi_old <- true_round(df$aqi_old, 0)
  df <- df %>% dplyr::select(-poll)
  colnames(df)[which(colnames(df) == "aqi_old")] <- paste0("aqi_", pollutant, "_old")
  colnames(df)[which(colnames(df) == "hcat_old")] <- paste0("hcat_", pollutant, "_old")
